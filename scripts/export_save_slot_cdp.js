#!/usr/bin/env node
// Export one h5mota save slot from the already-open Chrome CDP target.
...
function parseArgs(argv) {
  ...
  if (!out.out) out.out = path.join(ROOT, "outputs", "results", `slot${out.slot}_snapshot.json`);
  return out;
}
  ...
  try {
    const payload = await evalJson(
      ...
      `new Promise(function(resolve){
        try {
          core.getSave(${args.slot}, function(data){
            if (!data) return resolve(JSON.stringify({ok:false, slot:${args.slot}, reason:"missing"}));
            resolve(JSON.stringify({ok:true, slot:${args.slot}, data:data}));
          });
        } catch (e) {
    ...
    if (!payload.ok) throw new Error(`save${args.slot} export failed: ${JSON.stringify(payload)}`);
