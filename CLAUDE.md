...
This file mirrors `AGENTS.md` for Claude-compatible agents. Keep both files synchronized when project-level route, score, or workflow assumptions change.
...
Old `1482.5` artifacts are invalid because they allowed an MT10 special action before MT9 upFloor was actually reachable. The special-action guard now lives in `scripts/post9_resource_group_search.py::apply_action`.
...
- `_yd/_bd/_rd` are cumulative door consumption.
- Resource group score is for ordering and reporting, not a replacement for safe Pareto retention.
